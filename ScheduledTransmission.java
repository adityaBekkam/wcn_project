...
public class ScheduledTransmission {
	...
	public static void printPendingTransmissions(int timeStamp,ArrayList<Transmission>[] transfers){
		System.out.println("TRANSMISSION PENDING AFTER "+timeStamp+" seconds");
	...
	public static void allocateFragments(){
		...
		int lValue = 0 ;
		System.out.println("printing current paths used :");
			...
			if( !paths.get(i).isActive ){
				...
				if( randNum>qValues[i]  ){
					...
					for( int j=0 ; j<paths.get(i).pathSize()-1 ; j++ ){
						...
						else if( graph.nodes.get( paths.get(i).route.get(j)).techs.get(0)==1 && 1==graph.nodes.get( paths.get(i).route.get(j+1)).techs.get(0) ) {
	...
	public static int numFragsReceived(){
		...
		while( !isFinished ){
			...
			if( transfers[time].size()==0 ){
				...
				printPendingTransmissions(time,transfers);
				numOfTimes = 0;
				Transmission t ; 
				...
				for( int j=0 ; j<transfers[time].size() ; j++ ){
					...
					if( t.technology==1 ){
						...
						//System.out.println("Interference range of "+t.src+"->"+t.dest+": "+interferenceRange);
						Transmission tempTr ;
						for( int k=j+1 ; k<transfers[time].size() ; k++ ){
							...
							if( tempTr.technology==1 && graph.isPresentInInterferenceRange(t.src, t.dest, tempTr.src, tempTr.dest, interferenceRange) ){
								//System.out.println("Interference found between "+t.src+"-->"+t.dest+", "+tempTr.src+"-->"+tempTr.dest);
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		//tempArr2 --> adjacency matrix of the network.
		//tempArr1 --> tells whether there is an edge between a pair of nodes.
		...
		//This is to understand how interference is handled in single transmissions.
		//allocateFragments();
		//numFragsReceived();
	}
